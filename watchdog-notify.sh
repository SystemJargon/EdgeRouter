...
# Put the email message inline
      $sendmail -F "Edgemax Router" -f router@youremail.com $2 << EOF
To: $2
...
# something we could add instead of email
curl -X POST -H "Content-Type: application/json" -d "${status}" https://maker.ifttt.com/trigger/wan_change/with/key/{REMOVED}

# could replace curl to use non IFTTT also. This being because IFTTT moved to their premium and delay-if-free business model.